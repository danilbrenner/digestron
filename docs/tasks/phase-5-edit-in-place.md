...
- [x] **p5-rename-message-context** — Rename `MessageContext` to `CommandContext` across the solution  
...
- [x] **p5-context-message-id** — Add `LastMessageId` to `MessageContext`  
...
- [x] **p5-loading-stores-id** — `SendDigestLoadingMessageAsync` stores the sent message ID in context  
...
- [x] **p5-edit-digest-message** — Replace `SendDigestAsync` with `EditDigestMessageAsync` in the digest flow  
