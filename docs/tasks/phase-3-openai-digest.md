...
- [x] **p3-openai-packages** — Add OpenAI NuGet package  
  Add `OpenAI` to `Directory.Packages.props`. Reference from `Digestron.Infra`.

- [x] **p3-digest-interface** — Define IDigestService interface in Domain  
...
- [x] **p3-di-wiring-openai** — Wire OpenAI service into DI  
  Register `OpenAiOptions` and `OpenAiDigestService` (`IDigestService`) in DI with `AddInfra`.

- [x] **p3-digest-command** — Implement /digest command with real AI digest  
